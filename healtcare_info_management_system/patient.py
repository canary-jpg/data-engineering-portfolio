...
class Patient:
    ...
    def add_patient(self):
        ...
        self.zip_code = st.text_input('PIN code')
            ...
            with conn:
                c.execute(
                    ...
                    """,
                    {
                        ...
                        'phone_2': self.contact_number_2, 'uid': self.uid,
                        'weight': self.weight,
                        'height': self.height, 'address': self.address,
                        'city': self.city, 'state': self.state,
                        'pin': self.zip_code, 'kin_name': self.next_of_kin_name,
    ...
    def update_patient(self):
        ...
        if id == '':
            ...
            contact_number_2 = st.text_input('Alternate contact number (optional)')
            self.contact_number_2 = (lambda phone : None if phone == '' else phone)(contact_number_2)
            self.weight = st.number_input('Weight (in lb)', value = 0, min_value = 0, max_value = 400)
            self.height = st.number_input('Height (in ft)', value = 0, min_value = 0, max_value = 275)
            self.address = st.text_area('Address')
            self.city = st.text_input('City')
            self.state = st.text_input('State')
            self.zip_code = st.text_input('PIN code')
            ...
            if update:
                ...
                with conn:
                    c.execute(
                        ...
                        state = :state, zip = :zip_code, next_of_kin_name = :kin_name,
